...
def get_rebin_values(hist_s, hist_bg, N_bins_final: int = 10):
    ...
    for i in range(1, N_bins_input + 1):
        ...
        N_events += hist_s.GetBinContent(i)
        if N_events >= events_per_bin * bin_count:
            # when *N_events* surpasses threshold, append the corresponding bin edge and count
            logger.info(f"++++++++++ Append bin edge {bin_count} of {N_bins_final} at edge {hist_s.GetBinLowEdge(i)}")
            rebin_values.append(hist_s.GetBinLowEdge(i + 1))
            bin_count += 1

    # final bin is x_max
    x_max = hist_s.GetBinLowEdge(N_bins_input + 1)
...
class ModifyDatacardsFlatRebin(
    ...
    def run(self):
        ...
        with uproot.open(inp_shapes.fn) as file:
            ...
            # get the hists of only the bg processes of the given category
            hists_bg = nominal_hists.copy()
            del hists_bg[rebin_processes[0]], hists_bg['data_obs']
            ...
            rebin_values = get_rebin_values(hist, hists_bg, self.get_n_bins())
