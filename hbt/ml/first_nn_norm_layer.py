...
values/pairs per event -> mask is applicable to all arrays """
def create_pairs(jets, pairs_dict, pais_input, replace_val, max_jets):
    ...
    e = four_vectors[:, :, 0]
    masking_val = 2 * EMPTY_FLOAT # generate mask based on e that is applied to all,
    mask = (e != masking_val) # since all pairs (num of pairs per event equal for all)
    ...
    stacking = np.array([e, mass, pt, eta, phi, delta_eta, delta_phi])
    mask_tiled = np.array([mask] * len(stacking))
    stacking_masked = np.where(mask_tiled, stacking, replace_val).tolist()
...
def reshape_norm_inputs(events_dict, norm_features, input_features, n_op_nodes, n_op_deepSets, padding, baseline_jets, masking_val):
    ...
    mean_inputs2_pad = np.concatenate((np.full(n_op_deepSets, 0.), mean_inputs2))
    std_inputs2_pad = np.concatenate((np.full(n_op_deepSets, 1.), std_inputs2))
    sig3_pad = mean_inputs2 - 3 * std_inputs2
    pad1 = np.full_like(sig3_pad, -1.)
    if padding == "1":
        inputs2 = np.where(inputs2 == EMPTY_FLOAT, pad1, inputs2)
    elif padding == "3sig":
        inputs2 = np.where(inputs2 == EMPTY_FLOAT, sig3_pad, inputs2)
    ...
    norm_dict["mean_inputs2"] = tf.convert_to_tensor(mean_inputs2_pad, dtype=tf_dtype)
    norm_dict["std_inputs2"] = tf.convert_to_tensor(std_inputs2_pad, dtype=tf_dtype)
...
class SimpleDNN(MLModel):
    ...
    def prepare_inputs(
        ...
        train, train_norm = reshape_norm_inputs(train, self.norm_features, self.input_features, self.n_output_nodes, self.nodes_deepSets[-1], self.empty_overwrite, self.baseline_jets, self.masking_val)
        validation, _ = reshape_norm_inputs(validation, self.norm_features, self.input_features, self.n_output_nodes, self.nodes_deepSets[-1], self.empty_overwrite, self.baseline_jets, self.masking_val)
    ...
    def instant_evaluate(
        ...
        if self.model_type == "baseline":
            train["prediction"] = call_func_safe(model, train['input_baseline'])
            validation["prediction"] = call_func_safe(model, validation['input_baseline'])
    ...
    def train(
        ...
        DS_feedforward_config = {'nodes': self.nodes_ff, 'activations': self.activation_func_ff,
            'n_classes': self.n_output_nodes, 'mean': train_norm['mean_inputs2'],
            'std': train_norm['std_inputs2']}
        ...
        if self.model_type == "baseline":
            ...
            tf_train = [train['input_baseline'], train['target']]
            tf_validation = [validation['input_baseline'], validation['target']]
        elif self.model_type == "DeepSets":
            model = CombinedDeepSetNetwork(deepset_config_jets, DS_feedforward_config)
            tf_train = [[train['inputs'], train['inputs2']], train['target']]
            tf_validation = [[validation['inputs'], validation['inputs2']], validation['target']]
        elif self.model_type == "DeepSetsPP":
            model = CombinedDeepSetPairsParallelNetwork(deepset_config_jets, deepset_config_pairs, DS_feedforward_config)
